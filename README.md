...
> 💡 默认演示由规则策略决策，模型调用数为 **0**。接入并选择模型后，才会进行模型决策。所有浏览器标签页共享同一个本地实验。
...
- Key 只保存在本次服务进程内，不写入仓库或导出文件；重启后需重新填写。
...
准备做 GPT / Claude / MiniCPM 对比？见 [公平对比与复现说明](docs/COMPARISON.md)。接口已支持，当前没有宣称云端模型的实测成绩。
...
🍎 **MiniCPM 已在 M2 / 16 GB 上用 FP16 真正跑过推理**。当前完整状态提示词在三个任务的 seed 0 上仍是 **0/3 成功**：关闭概率门槛后会重复接近物体，尚未完成抓取。我们公开这些失败、延迟和完整轨迹，继续改进决策输入；接口接通不等于任务跑通。官方 Jev / Claude 目前通过协议测试，真实 API 成绩待接入 Key 后记录。
