...
🍎 **MiniCPM 已在 M2 / 16 GB 上用 FP16 真正跑过推理**。最新紧凑状态与候选位移提示在三个任务的 seed 0 上仍是 **0/3 成功**：两项因连续无进展自动停止，一项达到 12 步预算，尚未完成抓取。我们公开失败、耗时和完整轨迹；接口接通不等于任务跑通。GPT / Jev / Claude 目前验证的是模拟响应下的协议与异常处理，真实 API 成绩待配置后记录。
