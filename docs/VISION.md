...
视觉模式把“从 MuJoCo 读取物体坐标”换成“从相机图像估计物体坐标”，再把结构化结果交给决策模型。原来的仿真状态模式仍然保留，方便排查感知误差。

```text
仿真 RGB-D 相机 → 本地颜色检测与三维定位 → 阶段 / 动作候选 → 模型选择 → 执行与反馈
```

选择 RGB-D 观察模式后，在“视觉”页签切换 RGB 与深度图。感知在动作边界采样，同一仿真时刻复用已有观测；这里显示的是最近一次采样，不是独立连续推流。

## 相机怎样提供物体信息

当前使用工作台侧方斜俯视的 640 × 480 相机，检测已有场景中的红色方块、蓝色目标和黄色障碍物。检测器在 RGB 图像中寻找颜色区域，结合深度和相机标定估计位置。它预先知道方块边长为 4 cm，以可见表面推算方块中心和放置高度；这些尺寸与颜色不是模型从图片中学出来的。
