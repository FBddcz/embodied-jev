...
2. 打开 **模型对比**，选择 2–3 路模型，设置共同任务、种子、预演、概率门槛和动作预算。可选同一 API 的不同模型，也可使用不同平台的连接。
...
[OpenAI 模型页](https://developers.openai.com/api/docs/models/gpt-6-astra)列出 `gpt-6-astra`，[Anthropic 模型页](https://platform.claude.com/docs/en/models/overview)列出 `claude-fable-5-1`。中转平台可能另用带厂商前缀的 ID，以平台实际名称为准。这里填写模型 ID，而不是 Claude Code 或 Codex 等工具名。
...
- 使用相同的几何/接触观察和动作候选。阶段筛选、运动控制由代码提供，评估范围是这个系统中的模型选择能力。
